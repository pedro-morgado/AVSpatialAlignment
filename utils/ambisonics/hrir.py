...
import numpy as np
import os
from utils.ambisonics.common import Position
from utils.ioutils.av_wrappers import av_audio_loader
...
class CIPIC_HRIR(object):
    def __init__(self, dirname):
        ...
        for i, phi in enumerate(azimuth):
            ...
            self.right_hrir[:, i, :] = np.flip(av_audio_loader(os.path.join(dirname, right_fn))[0], axis=0)
            self.left_hrir[:, i, :] = np.flip(av_audio_loader(os.path.join(dirname, left_fn))[0], axis=0)
