...
from utils.ambisonics.hrir import CIPIC_HRIR
from utils.ambisonics.position import Position, PositionalSource, MovingSource
from scipy.ndimage.interpolation import shift
from utils.ambisonics.tdesigns import get_tDesign

C = 343.     # Speed of sound [m/s]

class VirtualMics(object):
    def __init__(self, mic_pos, delay=False, a=0.1, c=C):
    ...
    def binauralize(self, sources):
        ...
        signals = [0 for _ in range(len(self.mic_pos))]
        for src in sources:
            for i, mic in enumerate(self.mic_pos):
                dist = np.sqrt(((src.position.coords('cartesian') - mic.coords('cartesian'))**2).sum())
                mic_vec = mic.coords('cartesian')
                in_vec = mic_vec - src.position.coords('cartesian')
                cos_angle = (mic_vec * in_vec).sum() / ((mic_vec**2).sum() * (in_vec**2).sum())**0.5
                dir_att = (1 - cos_angle * (1 - self.a) + self.a) / 2.

                # Time delay
                if self.delay:
                    delay = int(dist / self.c * src.sample_rate)
                    signal = shift(src.signal, delay, cval=0.)
                else:
                    signal = src.signal
                ...
                signals[i] += dir_att * attn * signal / len(sources)
...
class AmbisonicBinauralizer(object):
    ...
    def binauralize(self, ambi):
        ...
        # Binauralize speaker as if they were point sources
        sources = list(map(lambda i: PositionalSource(speakers[:, i], self.speaker_pos[i], self.fmt.sample_rate),
